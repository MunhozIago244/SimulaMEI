...
import { consumeRateLimit, applyRateLimitHeaders } from '@/lib/security/rate-limit'
  ...
  try {
    ...
    if (!rateLimit.allowed) {
      return applyRateLimitHeaders(
        NextResponse.json({ error: 'Limite de tentativas atingido. Tente novamente mais tarde.' }, { status: 429 }),
        rateLimit,
    ...
    const tipo = body.tipo === 'contador_waitlist' ? 'contador_waitlist' : 'simulacao'
    const normalizedCnae = typeof body.cnae === 'string'
