...
import { calcularFatorR, determinarAnexo } from './fatorR'
...
export function simular(entrada: EntradaSimulacao): ResultadoSimulacao {
  const { faturamentoAcumulado, mesAtual, cnae, folhaMensal, tipoMei } = entrada
  ...
  if (elegivelFatorR && (anexoPadrao === 'V' || anexoPadrao === 'III')) {
    fatorRResult = calcularFatorR(folha12meses, rbt12)
  ...
  const presumido = calcularPresumido(rbt12, categoria, folhaMensal)

  // 7. Lucro Real - mesma categoria e folha (margem padrao de 30%)
  const real = calcularReal(rbt12, MARGEM_REAL_DEFAULT, categoria, folhaMensal)
  ...
  const custoIV = anexoAtual === 'IV'
    ? calcularCustoRealAnexoIV(rbt12, folhaMensal)
    : null
...
export { calcularFatorR, analisarFatorR, calcularProLaboreIdeal } from './fatorR'
