...
const {
  ...
  getClientIpMock,
  getUserAgentMock,
  sendAccountantLeadConfirmationMock,
  ...
  sendAccountantLeadConfirmationMock: vi.fn(),
...
vi.mock('@/lib/resend', () => ({
  sendAccountantLeadConfirmation: sendAccountantLeadConfirmationMock,
...
describe('/api/accountant-leads POST', () => {
  beforeEach(() => {
    ...
    sendAccountantLeadConfirmationMock.mockResolvedValue({ ok: true })
  ...
  it('upserts a valid accountant lead with metadata', async () => {
    ...
    expect(sendAccountantLeadConfirmationMock).toHaveBeenCalledWith({
      email: 'contato@prime.com.br',
      nomeEscritorio: 'Prime Contabilidade',
    })
