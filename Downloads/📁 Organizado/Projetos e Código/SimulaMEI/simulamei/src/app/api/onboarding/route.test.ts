...
function makeSupabaseMock(options?: {
  ...
  const profileUpdateSelectMock = vi.fn().mockResolvedValue({
    error: options?.profileUpdateError ?? null,
    data: options && 'profileUpdateData' in options ? options.profileUpdateData : [{ id: resolvedUser?.id ?? 'user-1' }],
  })
  const profileUpdateEqMock = vi.fn(() => ({ select: profileUpdateSelectMock }))
  const profileUpdateMock = vi.fn(() => ({ eq: profileUpdateEqMock }))
  const profileUpsertMock = vi.fn().mockResolvedValue({ error: options?.profileUpsertError ?? null })
...
function validPayload() {
  return {
    ...
    atividadesRealizadas: 'Desenvolvimento de sistemas e consultoria técnica',
  }
}
...
describe('/api/onboarding POST', () => {
  ...
  it('returns 500 when profile persistence fails', async () => {
    createClientMock.mockResolvedValue(makeSupabaseMock({
      profileUpdateData: [],
      profileUpsertError: { message: 'upsert failed' },
    }).client)
  ...
  it('stores the profile and bootstrap simulation for valid onboarding payloads', async () => {
    ...
    expect(supabase.profileUpdateMock).toHaveBeenCalledWith(expect.objectContaining({
      ...
      atividades_realizadas: 'Desenvolvimento de sistemas e consultoria técnica',
    }))
    expect(supabase.profileUpdateEqMock).toHaveBeenCalledWith('id', 'user-1')
    expect(supabase.profileUpdateSelectMock).toHaveBeenCalledWith('id')
    expect(supabase.profileUpsertMock).not.toHaveBeenCalled()
