...
interface OfficeClientsTable {
  select<T = OfficeClientRow>(columns: string, options?: { count?: 'exact'; head?: boolean }): SupabaseQuery<T>
...
> {
  ...
  if (error) {
    console.error('[/api/accountant/clients/[id]] office query error:', error)
...
export async function GET(_request: NextRequest, context: RouteContext) {
  try {
    ...
    if (error) {
      console.error('[/api/accountant/clients/[id]] get error:', error.message)
    ...
    console.error('[/api/accountant/clients/[id]] GET error:', error)
    return NextResponse.json({ error: 'Erro interno ao carregar cliente.' }, { status: 500 })
  }
}
...
export async function PATCH(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (parsed.value.ativo === true) {
      ...
      if (current.error) {
        console.error('[/api/accountant/clients/[id]] current client error:', current.error.message)
        return NextResponse.json({ error: 'Não foi possível validar o cliente.' }, { status: 500 })
      }
      if (!current.data) {
        return NextResponse.json({ error: 'Cliente não encontrado.' }, { status: 404 })
      }
      ...
      if (!current.data.ativo) {
        ...
        if (activeCountResult.error) {
          console.error('[/api/accountant/clients/[id]] active count error:', activeCountResult.error.message)
    ...
    if (updateResult.error || !updateResult.data) {
      console.error('[/api/accountant/clients/[id]] update error:', updateResult.error?.message)
    ...
    console.error('[/api/accountant/clients/[id]] PATCH error:', error)
...
export async function DELETE(_request: NextRequest, context: RouteContext) {
  try {
    ...
    if (updateResult.error || !updateResult.data) {
      console.error('[/api/accountant/clients/[id]] delete error:', updateResult.error?.message)
      return NextResponse.json({ error: 'Não foi possível pausar o cliente.' }, { status: 500 })
    }

    return NextResponse.json({ ok: true, client: updateResult.data })
  } catch (error) {
    console.error('[/api/accountant/clients/[id]] DELETE error:', error)
    return NextResponse.json({ error: 'Erro interno ao pausar cliente.' }, { status: 500 })
  }
}
