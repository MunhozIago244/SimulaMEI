...
import { sendAccountantLeadConfirmation, sendAccountantLeadNotification } from '@/lib/resend'
...
export async function POST(request: NextRequest) {
  ...
  if (!process.env.NEXT_PUBLIC_SUPABASE_URL || !process.env.SUPABASE_SERVICE_ROLE_KEY) {
    console.error('[/api/accountant-leads] FATAL: Supabase env vars ausentes', {
  ...
  try {
    ...
    if (!rateLimit.allowed) {
      return applyRateLimitHeaders(
        NextResponse.json({ error: 'Limite de solicitações atingido. Tente novamente mais tarde.' }, { status: 429 }),
        rateLimit,
    ...
    const { error: dbError } = await (admin as ReturnType<typeof createAdminClient>)
    ...
    if (dbError) {
      ...
      console.error('[/api/accountant-leads] Supabase upsert error:', {
        message: e.message,
        code: e.code,
        details: e.details,
        hint: e.hint,
      })
      return NextResponse.json({ error: 'Não foi possível registrar o interesse agora.' }, { status: 500 })
    }
    ...
    void sendAccountantLeadConfirmation({
      ...
      console.error('[/api/accountant-leads] confirmation email error:', confError)
    })
    ...
    if (lead.carteiraRange === '150+') {
      void sendAccountantLeadNotification({
        ...
        console.error('[/api/accountant-leads] notification error:', notifError)
      })
    }
    ...
    console.error('[/api/accountant-leads] Unhandled error:', message)
    return NextResponse.json({ error: 'Erro interno ao registrar interesse.' }, { status: 500 })
  }
}
