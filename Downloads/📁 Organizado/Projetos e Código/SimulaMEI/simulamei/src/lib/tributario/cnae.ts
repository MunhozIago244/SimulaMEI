...
function comMetadataFiscal(info: CnaeInfo): CnaeInfo {
  const fator_r_aplicavel = info.elegivelFatorR
  const anexo_fixo = info.elegivelFatorR
    ? 'III_ou_V'
    : info.anexoPadrao === 'III' || info.anexoPadrao === 'V'
      ? info.anexoPadrao
      : undefined
  ...
  return {
    ...info,
    fator_r_aplicavel,
    ...(anexo_fixo ? { anexo_fixo } : {}),
  }
}

function toCnaeInfo(record: CnaeOficialRecord): CnaeInfo {
  const curado = CNAE_INDEX.get(record.cnae)
  if (curado) return comMetadataFiscal({ ...curado, classificacaoTributaria: 'curada' })

  return comMetadataFiscal({
    cnae: record.cnae,
...
export function getCnae(cnae: string): CnaeInfo | undefined {
  ...
  if (curado) return comMetadataFiscal(curado)
...
export function buscarCnaes(termo: string): CnaeInfo[] {
  ...
  if (!t) return CNAE_MAP.slice(0, 10).map(comMetadataFiscal) // top 10 padrão
  ...
  return [...curados.map(comMetadataFiscal), ...oficiais].slice(0, 15)
}
...
export function getCnaesDestaque(): CnaeInfo[] {
  return CNAE_MAP.filter(c => c.categoria === 'ti_consultoria').slice(0, 8).map(comMetadataFiscal)
}
...
export function getCnaesAgrupados(maxPorGrupo = 6): Record<CnaeCategoria, CnaeInfo[]> {
  ...
  for (const cat of CATEGORIA_ORDEM) {
    result[cat] = CNAE_MAP.filter(c => c.categoria === cat).slice(0, maxPorGrupo).map(comMetadataFiscal)
  }
  return result
}
