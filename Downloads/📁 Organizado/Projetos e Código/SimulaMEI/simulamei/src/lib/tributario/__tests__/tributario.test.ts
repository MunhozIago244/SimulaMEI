...
import { analisarFatorR, calcularFatorR, calcularFolhaFatorR, calcularProLaboreIdeal, determinarAnexo } from '../fatorR'
...
describe('motor tributario', () => {
  ...
  it('includes CLT payroll, automatic charges, RPA and benefits in Fator R', () => {
    const semClt = simular({
      faturamentoAcumulado: 75_000,
      mesAtual: 6,
      cnae: '6201-5/01',
      folhaMensal: 0,
      folhaDetalhada: { proLabore: 2_000 },
      tipoMei: 'geral',
    })
    const comClt = simular({
      faturamentoAcumulado: 75_000,
      mesAtual: 6,
      cnae: '6201-5/01',
      folhaMensal: 0,
      folhaDetalhada: { proLabore: 2_000, salariosClt: 3_000, rpa: 500, beneficios: 300 },
      tipoMei: 'geral',
    })

    expect(calcularFolhaFatorR({ proLabore: 2_000, salariosClt: 3_000 }).inssPatronal).toBe(1_000)
    expect(calcularFolhaFatorR({ proLabore: 2_000, salariosClt: 3_000 }).fgts).toBe(240)
    expect(comClt.fatorR?.fatorR).toBeGreaterThan(semClt.fatorR?.fatorR ?? 0)
    expect(comClt.entrada.folhaMensal).toBe(7_040)
  })
  ...
  it('exposes Fator R applicability metadata for CNAE badges', () => {
    expect(getCnae('6201-5/01')?.fator_r_aplicavel).toBe(true)
    expect(getCnae('6201-5/01')?.anexo_fixo).toBe('III_ou_V')
    expect(getCnae('8599-6/04')?.fator_r_aplicavel).toBe(false)
    expect(getCnae('8599-6/04')?.anexo_fixo).toBe('III')
  })
})
