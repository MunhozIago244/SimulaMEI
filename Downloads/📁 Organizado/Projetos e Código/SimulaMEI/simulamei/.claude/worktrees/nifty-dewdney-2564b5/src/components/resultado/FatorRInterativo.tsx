...
export function FatorRInterativo({ projecao, fatorRInicial }: FatorRInterativoProps) {
  ...
  const resultado = calcularFatorR(prolabore * 12, projecao)
  const fr = resultado.fatorR
  const atingeMinimo = fr >= FATOR_R_MINIMO

  // Quanto falta para chegar em 28%
  const prolaboreMinimo = (FATOR_R_MINIMO * projecao) / 12
  const falta = prolaboreMinimo - prolabore
  ...
  return (
    <div style={{
      ...
      <p style={{ fontSize: 13, color: 'var(--text2)', marginBottom: 24 }}>
        Arraste o pró-labore e veja o impacto no regime.
      </p>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 20 }}>
        <div>
          <div style={{ fontSize: 11, color: 'var(--text3)', marginBottom: 2 }}>Pró-labore mensal</div>
          <MonoVal size={28}>{fmt(prolabore)}<span style={{ fontSize: 14, fontWeight: 400, color: 'var(--text2)' }}>/mês</span></MonoVal>
        </div>
        ...
        <div style={{ position: 'relative' }}>
          ...
          <input
            ...
            onChange={e => {
              ...
              setProlabore(Number(e.target.value))
            }}
            style={{ position: 'relative', zIndex: 1, background: 'transparent' }}
            aria-label="Pró-labore mensal interativo"
          />
        </div>
      ...
      <div style={{
        ...
        {atingeMinimo ? (
          ...
          <div style={{ fontSize: 13, color: 'var(--orange)', lineHeight: 1.5 }}>
            <b>Anexo V aplicável.</b> Para atingir 28% (Anexo III), aumente o pró-labore em{' '}
