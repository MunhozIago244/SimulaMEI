...
> {
  ...
  if (error) {
    console.error('[/api/accountant/clients] office query error:', error)
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (error) {
      console.error('[/api/accountant/clients] list error:', error.message)
    ...
    console.error('[/api/accountant/clients] GET error:', error)
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (activeCountResult.error) {
      console.error('[/api/accountant/clients] count error:', activeCountResult.error.message)
    ...
    const insertResult = await table
      ...
      .select('id, name, email, cnae, tipo_mei, uf, municipio, ativo, inactive_reason, created_at, updated_at')
      .single()

    if (insertResult.error || !insertResult.data) {
      console.error('[/api/accountant/clients] insert error:', insertResult.error?.message)
    ...
    console.error('[/api/accountant/clients] POST error:', error)
