...
import { PLAN_ACCENT_COLORS, PLAN_DESCRIPTIONS, PLAN_LABELS } from '@/constants/plans'
...
export default async function DashboardPage() {
  ...
  if (!isOnboardingComplete(profileData as UserProfileOnboarding | null)) {
    redirect('/onboarding')
  }
      ...
      <div style={{ maxWidth: 1220, margin: '0 auto' }}>
          ...
          <Panel style={{ padding: 28, minHeight: 260 }}>
            ...
            <div className="dashboard-radar-metrics" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 12 }}>
              {[
                ...
                <div key={label} style={{ background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: 16 }}>
        ...
        <section className="dashboard-kpi-grid" style={{
          ...
          {[
            ...
            <Panel key={label} style={{ padding: 20 }}>
              <div style={{ color: 'var(--text3)', fontSize: 11, fontWeight: 800, textTransform: 'uppercase', marginBottom: 8 }}>
                {label}
              </div>
        ...
        <section className="dashboard-module-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 16, marginBottom: 16 }}>
          {[
            ['Monitor mensal', 'Entradas mensais ativam histórico, DAS estimado, rotina fiscal e leitura de anexo por ciclo.', 'Ativo'],
            ['API para contadores', 'Expor simulação e classificação fiscal por chave segura, com logs e limite de uso.', 'Planejado'],
            ['Relatório premium', 'Converter oportunidades em um relatório PDF com memória de cálculo e evidências.', 'Monetização'],
          ].map(([title, body, status]) => (
            <Panel key={title} style={{ padding: 22 }}>
              <Pill>{status}</Pill>
              <h2 style={{ fontSize: 18, margin: '14px 0 8px' }}>{title}</h2>
              <p style={{ color: 'var(--text2)', fontSize: 13, lineHeight: 1.7, margin: 0 }}>{body}</p>
            </Panel>
          ))}
        </section>
