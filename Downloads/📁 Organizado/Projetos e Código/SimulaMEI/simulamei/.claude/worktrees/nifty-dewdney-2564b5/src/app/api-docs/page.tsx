...
export default function ApiDocsPage() {
  const baseUrl = 'https://simulamei.com.br/api/v1'
  ...
  return (
    ...
    >
      ...
      <p style={{ marginBottom: 16 }}>
        A API pública está em beta privado. Quando liberada, as requisições usarão API Key no header{' '}
        <Code>X-API-Key</Code>. A geração de chaves ficará dentro do dashboard logado.
      </p>
      <Pre>{`curl -X POST ${baseUrl}/simular \\
  -H "X-API-Key: smei_sua_chave_aqui" \\
  ...
  -d '{ ... }'`}</Pre>
      ...
      <Pre lang="bash">{`curl -X POST ${baseUrl}/simular \\
  -H "X-API-Key: smei_sua_chave_aqui" \\
  -H "Content-Type: application/json" \\
  -d '{
