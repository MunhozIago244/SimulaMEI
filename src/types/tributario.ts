...
export interface ResultadoFatorR {
  ...
  folhaMinimaAnual: number
  folhaMinimaMensal: number
  aumentoFolhaMensalNecessario: number
  economiaAnual: number  // diferenca entre Anexo V e III se elegivel
  memoriaCalculo: {
    aliquotaEfetivaAnexoV: number
    aliquotaEfetivaAnexoIII: number
    rbt12Projetado: number
    diferencaAliquota: number
    economiaAnual: number
  }
  folhaDetalhada?: FolhaCalculada
  alertaProLabore?: AlertaProLabore
}
...
export interface AlertaProLabore {
  proLaboreMensal: number
  folhaMensal: number
  percentualFolha: number
  inssPessoalEstimado: number
}
