...
export async function POST(req: NextRequest) {
  try {
    ...
    const { faturamentoAcumulado, mesAtual, cnae, folhaMensal, folhaDetalhada, tipoMei } = body
    ...
    if (user) {
      try {
        ...
        if (!countError && (profile?.plano ?? 'free') === 'free' && (count ?? 0) >= FREE_SIMULATION_LIMIT) {
          return NextResponse.json(
            {
        ...
        console.warn('[/api/simular] Free-plan usage check skipped:', error)
      }
    }
