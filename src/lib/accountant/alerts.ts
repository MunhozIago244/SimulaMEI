...
export function getTetoAlertType(
  ...
  if (percentualExcesso > 0.20 || percentualUtilizado > 1.20) return 'teto_excesso_grave'
  if (percentualUtilizado >= 1) return 'teto_100'
  if (percentualUtilizado >= 0.95) return 'teto_95'
  if (percentualUtilizado >= 0.80) return 'teto_80'
  if (percentualUtilizado >= 0.70) return 'teto_70'
  return null
}
...
export function describeOfficeAlert(
  tipo: OfficeAlertType,
  payload: Pick<OfficeAlertPayload, 'clientName' | 'percentualUtilizado' | 'projecaoAnual' | 'tetoAnual'>,
) {
...
export function buildOfficeAlertCandidate({
  ...
  const tipo = getTetoAlertType(alerta.percentualUtilizado, alerta.percentualExcesso)
  if (!tipo) return null
